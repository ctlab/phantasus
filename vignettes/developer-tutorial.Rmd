...
- `gui` -- field's description, represented as an array of JSON-objects, which have following values:

    - `name` -- field's name;
    - `value` -- default value;
    - `type` -- field's type (most used types are: select, checkbox-list, bootstrap-list, text);
    - `multiple` -- if multiple values may be chosen (relevant for checkbox-list);
    - 'options' -- an array of values to choose from (relevent for lists and select);
...
- `execute` -- main method of the tool, which includes:
    ...
    2) processing of additional arguments, if they need to be derived from the dataset and input;
    ...
    es.then(function (essession) {
      ...
      var req = ocpu.call("methodName", args, function (session) {
        session.getObject(function (success) {
          ...
          // JSON:
          var result = JSON.parse(success); // after that you can proceed with result
