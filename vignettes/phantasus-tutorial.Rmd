...
**Please note**: Be advised that self-hosted Phantasus requires additional steps to make **FGSEA** work properly. Consider the end of this part for instructions. Your mileage may vary.
...
**Additional steps**: **FGSEA** requires pathway database in .rds files under cacheDir/fgsea folder. Pathway database is a .rds file containing dataframe with columns: geneID, pathName, geneSymbol. You can see example dataframe by entering
```{r, eval=FALSE}
data("dbExample", package="phantasus")
```
...
### Annotation convertation with AnnotationDB

**Please note**: Be advised that self-hosted Phantasus requires some additional installation to make **AnnotationDB** work properly. Consider the end of this part for instructions.

AnnotationDB provides an opportunity to convert annotation columns in dataset. Open _Tools/Annotate/Annotate Rows/From Database_. Example: converting ENTREZID column in GSE53986 to ENSEMBL. Select _org.Mm.hg.sqlite_ specimen database which column Gene ID, Source column type ENTREZID and result column type to ENSEMBL. 

```{r, out.width = "600px", echo = FALSE}
knitr::include_graphics("images/annotation_tool.png")
```

Clicking OK will append new column with correspoding ENSEMBL to gene annotation.

```{r, out.width = "600px", echo = FALSE}
knitr::include_graphics("images/annotation_result.png")
```

**Addtitional steps**: AnnotationDB tool requires annotation databases under cacheDir/annotationdb folder. For example you can get [Mus Musculus](https://bioconductor.org/packages/release/data/annotation/html/org.Mm.eg.db.html) database. Extract org.Mm.eg.sqlite and put to cacheDir/annotationdb folder.
