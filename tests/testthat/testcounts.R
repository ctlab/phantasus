...
test_that("loadCounts returns result", {
    ...
    count_files <- list.files(counts_dir, recursive = TRUE, full.names = TRUE)
    tmp_count_files <- sub(pattern = counts_dir, replacement = tmp_counts, x = count_files )
    file.copy(count_files, tmp_count_files)
    real_wd <- getwd()
...
test_that("updateCountsMeta create good meta files for archs4", {
    ...
    dir.create(tmp_counts)
    dir.create(file.path(tmp_counts, "archs4"))
    count_files <- list.files(counts_dir, recursive = TRUE, full.names = TRUE, pattern = ".h5$")
    tmp_count_files <- sub(pattern = counts_dir, replacement = tmp_counts, x = count_files )
    file.copy(count_files, tmp_count_files)
